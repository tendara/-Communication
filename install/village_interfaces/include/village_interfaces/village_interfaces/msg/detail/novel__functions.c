  ...
  village_interfaces__msg__Novel__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    village_interfaces__msg__Novel * data =
      (village_interfaces__msg__Novel *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!village_interfaces__msg__Novel__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          village_interfaces__msg__Novel__fini(&output->data[i]);
        }
        return false;
      }
    }
